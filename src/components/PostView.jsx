...
import {
  ...
  Heart, MessageCircle, Send, Bookmark, Music, Download, Loader2, CheckCircle, XCircle
} from 'lucide-react';
...
export default function PostView({
  ...
  // Share all slides to Instagram via n8n webhook
  const shareAllSlides = async () => {
      ...
      <div className="px-4 py-3 bg-white">
        ...
        <div className="flex justify-between mb-3">
          <div className="flex gap-4 items-center">
            ...
            <button
              onClick={shareAllSlides}
              disabled={isSharing}
              title="Share all slides to Instagram via n8n"
              className="relative flex items-center gap-1.5 disabled:opacity-50"
            >
              {isSharing ? (
                <Loader2 size={26} className="text-pink-500 animate-spin" />
              ) : shareStatus === 'success' ? (
                <CheckCircle size={26} className="text-green-500" />
              ) : shareStatus === 'error' ? (
                <XCircle size={26} className="text-red-500" />
              ) : (
                <Send size={26} className="text-black hover:text-pink-500 transition-colors" />
              )}
            </button>
          </div>
          <div className="flex items-center gap-3">
        ...
        {shareStatus === 'error' && (
          <p className="text-xs text-red-500 text-center mb-2">Share failed — check n8n webhook URL in server</p>
        )}
